...
export default function BlogCard({ blog }: BlogCardProps) {
  console.log(blog.field_blogimage.field_media_image.uri.url);
  const blogimage = process.env.DRUPAL_BASE_URL + blog.field_blogimage.field_media_image.uri.url;
    return (
      <div className="group flex flex-col gap-4 rounded-3xl border border-[#e7edf3] bg-white p-4 shadow-sm transition-all hover:shadow-md dark:border-slate-800 dark:bg-slate-900">
        ...
        <div className="px-2 pb-2">
          <div className="mb-3 flex items-center gap-2">
            ...
            <span className="text-xs text-[#4c739a]">{blog.field_estimated_read_time}</span>
          </div>
